...
class TodoContentParent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      newEdit: false,
  ...
  render() {
    ...
    return (
      ...
      <div className="TDParent">
        ...
        <div>
          {this.state.newEdit ? (
            <TodoEdit todoOut={this.onEditCheck} />
          ) : (
            <div />
          )}
        </div>
        <div>{mapToComponents(this.props.todoData)}</div>
      </div>
    );
  }
}

TodoContentParent.propTypes = propTypes;
TodoContentParent.defaultProps = defaultProps;

export default TodoContentParent;
